...
A deferred action is logged **once** in `decisions` with `decision='defer'` and `source='auto_defer'`. It is also inserted into `deferred` as the review queue entry. The `deferred` table is a mutable queue; the `decisions` table is the immutable audit log. Stats queries always operate on `decisions` only.

### Decision: Extended `source` enum

The `decisions.source` column (per CLAUDE.md: `user | rule | prediction | auto_afk`) is extended with one additional value:

| Value | Meaning |
|---|---|
| `user` | User approved/denied manually |
...
Step 7 — Smart AFK fallback     [MERGED: was steps 7 + 8 + 9]
  ...
  AFK OFF + notifications          → race(phone/telegram response, waitMs)
...
| Deferred relationship | Ambiguous | `decision='defer', source='auto_defer'` in decisions + queue row in deferred |
| `source` enum | 4 values | 5 values: adds `auto_defer` |
